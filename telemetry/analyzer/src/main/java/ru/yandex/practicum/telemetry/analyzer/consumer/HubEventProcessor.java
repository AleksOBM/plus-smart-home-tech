...
import ru.yandex.practicum.telemetry.analyzer.controller.HubEventController;
...
public class HubEventProcessor implements Runnable {
	...
	@Value("${kafka.hub-event-consumer-config.topics}")
	private String topic;

	private final KafkaConsumer<String, HubEventAvro> consumer;

	private final HubEventController controller;
		...
		while (running) {
			...
			for (ConsumerRecord<String, HubEventAvro> record : records) {
				try {
					...
					controller.updateHubData(record.value());
